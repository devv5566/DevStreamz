    ...
    return new Promise((resolve, reject) => {
        ...
        const req = protocol.request(requestOptions, (res) => {
            ...
            res.on('end', () => {
                ...
                if ((res.statusCode >= 300 && res.statusCode < 400) && res.headers.location && options.allowRedirects !== false && redirectCount < 5) {
                    const redirectUrl = res.headers.location.startsWith('http') 
                        ? res.headers.location 
                        : `${urlObj.protocol}//${urlObj.host}${res.headers.location}`;
                    console.log(`Following redirect (${res.statusCode}): ${url} -> ${redirectUrl}`);
                    return makeRequest(redirectUrl, options, redirectCount + 1)
                        .then(resolve)
                        .catch(reject);
                }
