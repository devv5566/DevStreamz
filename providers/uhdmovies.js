...
// --- Domain Fetching ---
let uhdMoviesDomain = 'https://uhdmovies.ink'; // Updated fallback domain
...
const browserRequest = async (url, options = {}) => {
  log(`[UHDMovies] Browser request via Puppeteer for: ${url}`);
  ...
  try {
    ...
    await page.setExtraHTTPHeaders({
      ...
      'Upgrade-Insecure-Requests': '1',
      'Referer': options.referer || 'https://google.com'
    });
    ...
    return { status, headers, data: content };
  } finally {
    await browser.close();
  }
};
...
const makeRequest = async (url, options = {}) => {
  ...
  if (!requestOptions.validateStatus) {
  ...
  if (UHDMOVIES_PROXY_URL) {
    ...
    return axiosInstance.get(proxiedUrl, requestOptions);
  }
  ...
  const blocked = response.status === 403 ||
    (typeof body === 'string' && (body.includes('Just a moment') || body.includes('cf-browser-verification') || body.includes('window._cf_chl_ctx')));

  if (blocked) {
    log('[UHDMovies] Detected Cloudflare challenge, retrying with Puppeteer...');
    return browserRequest(url, options);
  }
...
async function searchMovies(query) {
  try {
    ...
    let response = await makeRequest(searchUrl);
    if (response.status === 403 || (typeof response.data === 'string' && response.data.includes('Just a moment'))) {
      log('[UHDMovies] Search page blocked, retrying with browser fetch and fallback query format...');
      response = await makeRequest(`${baseUrl}/?s=${encodeURIComponent(query)}`, { browser: true, referer: `${baseUrl}/` });
    }
